...
<!-- TODO handle NA vs 0 in processed and missed so that processed_pct is numeric  -->
...
x$surveys %>% 
  wastdr::filter_realsurveys() %>% 
  plot_survey_count(placename = pn, prefix = pf, export = FALSE)
```

```{r svy_plt_hrs}
x$surveys %>% 
  wastdr::filter_realsurveys() %>% 
  plot_survey_effort(placename = pn, prefix = pf, export = FALSE)
```
...
-   add figure “Clutch Frequency”: show estimated number of turtles that lay 
    1 clutch, 2 clutches, 3 clutches, etc
-->
