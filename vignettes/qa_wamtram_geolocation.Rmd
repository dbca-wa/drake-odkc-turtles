...
if (fs::file_exists(w2_file)){ 
  ...
  w2_data <- wastdr::download_w2_data(save=w2_file, use_rodbc = FALSE)
}
...
invalid_coords <- w2_data$enc %>% 
  dplyr::filter(longitude < -180 | longitude > 180 | latitude < -90 | latitude > 90)

unlikely_coords <- w2_data$enc %>% 
  dplyr::filter(longitude < 100 | longitude > 150 | latitude < -45 | latitude > 0)
