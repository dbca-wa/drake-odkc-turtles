...
wastd_nesting_type_by_season_age_species %>% 
  ...
  gt::tab_header(
    ...
    subtitle="Nesting by locality, species, nest age tallied by nesting success"
  ) %>%
  gt::cols_label(
    area_name = "Locality",
    season = "Season",
...
x$tracks %>% 
  ...
  gt::tab_header(
    ...
    subtitle="Emergences tallied by locality, season, and species"
  ) %>%
  gt::cols_label(
    N = "Emergences"
  ) 
...
survey_season_stats %>% 
  ...
  gt::tab_header(
    ...
    subtitle="Survey stats grouped by locality and season"
  )
```
...
Re-save those marked as "production" to auto-adjust end time.
Manually adjust other time spans as appropriate.
...
Adjust each survey's duration, save, and watch the success message in WAStD
for "evicted encounters". These need to be adopted by another survey.
...
(incorrectly matched) WAStD user can be transferred within a Locality
