...
upload_to_sharepoint <- function(wastd_reports = NULL) {
  ...
  for (src_fn in source_files) {
    upload_item_to_sharepoint(src_fn, dst)
  }
  ...
  for (src_fn in source_files_del) {
    upload_item_to_sharepoint(src_fn, dst_rio)
  }
  for (src_fn in source_files_wptp) {
    upload_item_to_sharepoint(src_fn, dst_rio)
  }
  # add del_rio when approved
  # for (src_fn in source_files_rio) {upload_item_to_sharepoint(src_fn, dst_rio)}
  for (src_fn in source_files_qa) {
    upload_item_to_sharepoint(src_fn, dst_rio)
  }
  ...
  source_files_qa <- fs::dir_ls(here::here("inst/reports"), regex = "qa_")
  for (src_fn in source_files_pth) {
    upload_item_to_sharepoint(src_fn, dst_cfh)
  }
  for (src_fn in source_files_qa) {
    upload_item_to_sharepoint(src_fn, dst_cfh)
  }
