test_that("odkc_tt_as_wastd_turtledmg works", {
  data("odkc_data", package = "wastdr")
  data("wastd_data", package = "wastdr")

  odkc_names <- odkc_data$tt %>%
    odkc_tt_as_wastd_turtledmg() %>%
  ...
  for (n in odkc_names) {
    testthat::expect_true(
      ...
      label = glue::glue("Column \"{n}\" exists in wastd_data$turtle_dmg")
    )
  }
})

# usethis::use_r("odkc_tt_as_wastd_turtledmg")
