...
x %>% save(file = fs::path(fn, "wastd_data.RData"))
```
...
objects is saved to an `.RData` file which can be read into R directly.
All code in this workbook can be reproduced by running the code shown
inline with the data from `.RData`.
...
Fan angle data is only exported here for now. The map shows nests with
recorded hatchling fans.
