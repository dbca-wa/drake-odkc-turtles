...
-   The WAMTRAM Users were downloaded on `r w2_data$downloaded_on`.
...
-   `legacy_username` - the ODK Collect username
...
For each mismatch: Find the correct WAStD user, and add the `legacy_username` to
...
validation_data_extracts <- 
  a %>%
  pointblank::get_data_extracts() 

if (length(validation_data_extracts > 0)) {
  validation_data <- validation_data_extracts %>%
  magrittr::extract2(1) %>%
  dplyr::rowwise() %>% 
  dplyr::arrange(-dist)
...
validation_data %>%
  ...
  "All users are matched beyond reasonable doubt." %>% 
    wastdr::wastdr_msg_success(verbose = TRUE)
}
```
