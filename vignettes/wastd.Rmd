...
```{r dist_map_2020}
tracks_min_admin <- x$tracks %>% 
  dplyr::select(species, source_id, absolute_admin_url)

x$nest_dist %>% 
  wastdr::filter_wastd_season(2020) %>%
  # dplyr::left_join(tracks_min_admin, by=c("encounter_source_id" = "source_id")) %>% 
  dplyr::rename(
    absolute_admin_url = encounter_absolute_admin_url
  ) %>% 
  wastdr::map_dist()
```
...
`nest_tags_with_encounters.csv`.

```{r nests_tagged}
tracks_min <- x$tracks %>% 
  dplyr::select(species, source_id)

wastd_tagged_nests <- x$nest_tags %>% 
  dplyr::left_join(tracks_min, by=c("encounter_source_id" = "source_id")) %>% 
  wastdr::exclude_training_species() 

wastd_tagged_nests %>%
  readr::write_csv(here::here(fn, "nest_tags_with_encounters.csv"))
```
