...
wastd_nesting_type_by_season_age_species <-
  dplyr::bind_rows(wastd_nesting_type_by_season_age_species_single %>% dplyr::add_rownames(), 
          wastd_nesting_type_by_season_age_species_tally %>% dplyr::add_rownames()) %>% 
...
trt_conn <- 
  ...
  dplyr::arrange(-season, place_code, species)
...
all_surveys %>% 
  ...
  gt::tab_header(
    ...
    subtitle="Survey stats grouped by rookery and season, dates are turtle dates"
  )
```
...
The low numbers in the 2017-18 season can be explained with the first two weeks
being captured on paper only pending backfilling of the data into WAStD.
...
## Morphometrics
<!-- TODO use `w2db$obs_measurements` -->
```{r}
trt_morph <- 
...
Tagging data contains morphometric measurements "curved carapace length" (`w2$ccl`)
and "curved carapace width" (`w2$ccw`).

Separately for each metric (ccl or ccw), records with existing measurements are 
grouped by season and species, then summarised into mean, sd, min, and max.

These summary statistics of curved carapace length and width are shown in 
Millimetres below.
...
trt_morph_ccl %>%
  dplyr::filter(Species == "Natator depressus") %>% 
  morph_gt("Flatback", "CCL")
```

```{r morph_fb_ccw}
trt_morph_ccw %>%
  dplyr::filter(Species == "Natator depressus") %>% 
  morph_gt("Flatback", "CCW")
```
...
trt_morph_ccl %>%
  dplyr::filter(Species == "Eretmochelys imbricata") %>% 
  morph_gt("Hawksbill", "CCL")
```

```{r morph_hb_ccw}
trt_morph_ccw %>%
  dplyr::filter(Species == "Eretmochelys imbricata") %>% 
  morph_gt("Hawksbill", "CCW")
```
...
trt_conn %>% 
  ...
  gt::gt(
    groupname_col=NULL,
    caption = "Connectivity of Flatback turtles in 2020-21"
  ) %>% 
  gt::tab_header(
    title="Connectivity of Flatback turtles in 2020-21"
  ) %>% 
  gt::cols_label(
    place_code = "Observed at"
  ) %>% 
  gt::tab_spanner(
    label = "Initially observed at",
    columns = c(
      -gt::contains("place_code")
    )
  ) 
```
...
```{r trt_conn_hb}
trt_conn %>% 
  dplyr::filter(species == "eretmochelys-imbricata", season==2020) %>%
  dplyr::select(-species, -season) %>% 
  gt::gt(
    groupname_col=NULL,
    caption = "Connectivity of Hawksbill turtles in 2020-21"
  ) %>% 
  gt::tab_header(
    title="Connectivity of Hawksbill turtles in 2020-21"
  ) %>% 
  gt::cols_label(
    place_code = "Observed at"
  ) %>% 
...
```{r trt_conn_gn}
trt_conn %>% 
  dplyr::filter(species == "chelonia-mydas", season==2020) %>%
  dplyr::select(-species, -season) %>% 
  gt::gt(
    groupname_col=NULL,
    caption = "Connectivity of Green turtles in 2020-21"
  ) %>% 
  gt::tab_header(
    title="Connectivity of Green turtles in 2020-21"
  ) %>% 
  gt::cols_label(
    place_code = "Observed at"
  ) %>% 
